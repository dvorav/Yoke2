  ...
  $.get("/api/chat", function (data) {
    if (data.length !== 0) {
      for (let i = 0; i < data.length; i++) {
        ...
        $("#room").append(
          '<option value="' + data[i].topic + '">' + data[i].topic + "</option>"
        );
      }
    }
  });

  //Local Storage Retrieve Styles
  function localStorageStyles() {
    let defaultCssStyle = "./assets/css/chatroom.css";
    let retrieve = localStorage.getItem("style");

    if (window.localStorage.length) {
      document.getElementById("pagestyle").setAttribute("href", retrieve);
    } else {
      document
        .getElementById("pagestyle")
        .setAttribute("href", defaultCssStyle);
    }
  }
    localStorageStyles();
